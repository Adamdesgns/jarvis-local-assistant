...
(function () {
  ...
  var FRAG_BODY = [
    ...
    '  // finishing pass: spectral limb fringe + palette grade + subtle flicker.',
    '  // Fringe frequencies and masks reuse the existing silhouette; flicker',
    '  // rates are integer multiples of W0 so the 120 s phase wrap is seamless.',
    '  float caOff = 2.5 * px;',
    '  float inR = 1.0 - smoothstep(-px, px, d - caOff);',   // red silhouette, slightly larger
    '  float inB = 1.0 - smoothstep(-px, px, d + 0.5 * caOff);', // blue silhouette, slightly smaller
    '  col += vec3(0.20, 0.02, 0.00) * max(inR - inMask, 0.0) * (0.3 + 0.5 * uEnergy);',
    '  col += vec3(0.00, 0.02, 0.18) * max(inMask - inB, 0.0) * (0.3 + 0.5 * uEnergy);',
    '  vec3 warm = col * vec3(1.15, 0.88, 0.62);',
    '  vec3 cool = col * vec3(0.88, 0.97, 1.12);',
    '  col = mix(col, mix(cool, warm, uMode), 0.22);',
    '  col *= 1.0 + 0.02 * sin(uTime * 573.0 * W0) * sin(uTime * 139.0 * W0);',
    '',
    '  col *= uFade;',
